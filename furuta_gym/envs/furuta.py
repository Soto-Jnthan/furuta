...
TH_ACT_LIM = 0.9 * np.pi  # rad (must be less or equal to np.pi)
MTR_STEP_ANGLE = 1.8  # degrees
MTR_MODE_DVSR = 16  # Power of 2 for the microstep mode
MTR_MAX_STEP_RATE = 500  # fullstep/s
...
class Furuta(gym.Env):
    def __init__(self, port, baudrate, fs):
        ...
        self.map_act = lambda a: a[0] * MTR_MAX_STEP_RATE * MTR_MODE_DVSR
    ...
    def step(self, a):
        ...
        self.uC.write_action(self.map_act(a_cmd))
        ...
        info = {
            "pendulum_angle": float(state[PHI]),
            ...
            "loop_time": float(self.timing.dt_actual),
        }  # policy output
    ...
    def read_state(self):
        ...
        if self.prev_state is not None:
            ...
            if dphi < -np.pi:  # adjust angular difference if needed
                ...
                dphi -= 2 * np.pi
            self.rel_phi += dphi
            vel = self.vel_filt(np.array([self.rel_phi, theta]))  # rad/s
            if self.timing.dt_actual != 0:  # adjust to time delays
                vel *= self.timing.dt / self.timing.dt_actual
        else:
            vel = np.array([0, 0])  # rad/s
            self.rel_phi = 0  # relative phi displacement (rad)
            self.vel_filt.set_initial_state(np.array([self.rel_phi, theta]))
        state = np.concatenate([pos, vel], dtype=np.float32)
    ...
    def get_obs(self, state):
        ...
        return np.array(
            [
                ...
                np.sin(state[THETA]),
                np.cos(state[THETA]),
                state[PHI_DOT] / PH_DOT_MAX,
                state[THETA_DOT] / TH_DOT_MAX,
            ],
            dtype=np.float32,
        )
